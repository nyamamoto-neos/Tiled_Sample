...
local function buildProperties( cache, tileset )
	print("#buildProperties", tileset.name)
	local tiles = tileset.tiles or {}
	...
	for _, tile in ipairs(tiles) do
		...
		if properties then
			...
			for name, value in pairs(properties) do 
				if not restricted_properties[name] then
					properties_copy[name] = value
					print("","",name, value)
				end
	...
	if tileset then
		...
		if image then
			...
			if image.hasBody then 
				...
				--print("physics.addBody", image.name, image.type)
...
function Map:new( filename, tilesets_dir, texturepacker_dir, _lua_dir )
    ...
    for i, tileset in ipairs( data.tilesets ) do
		if tileset.filename ~=nil then
			local Len =  tileset.filename:len()
			print(lua_dir..tileset.filename:sub(1,Len -4))
			local tileProps = require(lua_dir..tileset.filename:sub(1,Len -4))
